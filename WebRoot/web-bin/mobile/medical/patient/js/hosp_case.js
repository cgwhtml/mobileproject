...
function callBack(data){console.log(data);
	...
	if(data && data.data){
		...
		if(basic){
            ...
            if(basic.deptName){
                $("#base_info").append('&nbsp;'+basic.deptName);
            }
            if(basic.attendDrName){
                $("#base_info").append('&nbsp;'+$.trim(basic.attendDrName));
            }
            if(basic.admitDiagName){
                $("#base_info").append('&nbsp;'+basic.admitDiagName);
            }
		...
		if(data.data.medicalSurgeryRecords && data.data.medicalSurgeryRecords.length!=0){
			...
			$(".toOperation").bind("click",function(){
            	...
            	// var result={
            		// url:window.location.search,
            	var	result=data.data.medicalSurgeryRecords[index];
            	// };
            	result=$.param(result);
            	console.log(result);
            	result="?"+result;
