...
$(function(){
	...
	$.ajax({
		...
		timeout:60000,
		type:"get",
		url: LOCALHOST_URL+'/web-bin/m/medical/hospital/query_visitList_data'+query,
		dataType:"json",
...
//		location.href ="/HugPage/web-bin/mobile/medical/hospital/medicalRecordList.html"+back;
//	});
//	
...
function callBack(data){
	...
	if(data!=null && data.data!=null){
		...
		if(data.data.outhospFee!=null){
			...
			$("#toCost").html(
				'<p id="p4">医保类别：'+data.data.outhospFee.medicalCategory+'<img src="/web-bin/mobile/medical/hospital/images/blue.png"></p>'+
				'<div id="cost">'+
					'<img alt="" src="/web-bin/mobile/medical/hospital/images/cost.png">'+
					'<div>'+
						'<p>总费用</p>'+
			...
			$("#toCost").bind("click",function(){
				...
				location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_cost_page'+result;
			});	
		}else{
			$("#fee").hide();
		}
		...
		if(data.data.orderInfos!=null && data.data.orderInfos.length!=0){
			...
			$(".medicine_list").html(
				...
				'{#foreach $T as Row}'+
					'<div class="toMedicine">'+
						...
						'<p>{$T.Row.drugName}</p>'+
						'<img src="/web-bin/mobile/medical/hospital/images/blue.png">'+
					'</div>'+
				'{/for}'+
				'</textarea></p>'+
				'<div class="s4"></div>'
			...
			for(var i=0;i<length;i++){
				...
				$.ajax({
					...
					on: true,
					type:"get",
					url: LOCALHOST_URL+'/web-bin/m/medical/hospital/query_visitOrder_data'+id,
					dataType:"json",
					success: function(data1){
						...
						$("#m"+i).find(".toMedicine").bind("click",function(){
							...
							location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_visit_medicine_page'+data2;
						});	
					},
				});
			}
		...
		if(data.data.examReqs!=null && data.data.examReqs.length!=0){
			...
			$(".toExamination").bind("click",function(){
				...
				location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_examination_page'+result;
			});	
		}else{
			$("#examination").hide();
		}
		...
		if(data.data.testReqs!=null && data.data.testReqs.length!=0){
			...
			$(".toInspection").bind("click",function(){
				...
				location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_inspection_page'+result;
			});	
		}else{
			$("#inspect").hide();
		}
