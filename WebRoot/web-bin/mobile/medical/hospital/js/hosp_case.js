...
$(function(){
	...
	$.ajax({
		...
		timeout:60000,
		type:"get",
		url: LOCALHOST_URL+'/web-bin/m/medical/hospital/query_hospList_data'+query,
		dataType:"json",
...
function callBack(data){
	...
	if(data!=null && data.data!=null){
		...
		if(data.data.inhospFee!=null){
			...
			$("#toCost").html(
				'<p id="p4">医保类别：'+data.data.inhospFee.medicalCategory+'<img src="/web-bin/mobile/medical/hospital/images/blue.png"></p>'+
				'<div id="cost">'+
		...
		if(data.data.orders!=null && data.data.orders.length!=0){
			...
			$(".toMedicine").bind("click",function(){
				...
				location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_hosp_medicine_page'+record;
			});	
		...
		if(data.data.surgeryRecords!=null && data.data.surgeryRecords.length!=0){
			...
			$(".toOperation").bind("click",function(){
				...
				location.href = LOCALHOST_URL+'/web-bin/m/medical/hospital/to_hosp_operation_page'+result;
			});	
