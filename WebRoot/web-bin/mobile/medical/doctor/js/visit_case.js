...
$(function(){
	...
	$.ajax({
		...
		timeout:60000,
		type:"get",
		url: LOCALHOST_URL+'/web-bin/m/medical/doctor/query_visitList_data'+query,
		dataType:"json",
...
function callBack(data){
	...
	if(data!=null && data.data!=null){
		...
		if(data.data.outhospFee!=null){
			...
			$("#toCost").html(
				'<p id="p4">医保类别：'+data.data.outhospFee.medicalCategory+'<img src="/HugPage/web-bin/mobile/medical/doctor/images/blue.png"></p>'+
				'<div id="cost">'+
		...
		if(data.data.orderInfos!=null && data.data.orderInfos.length!=0){
			...
			$(".medicine_list").html(	
				...
				'{#foreach $T as Row}'+
					'<div class="toMedicine">'+
						...
						'<p>{$T.Row.drugName}</p>'+
						'<img src="/web-bin/mobile/medical/doctor/images/blue.png">'+
					'</div>'+
				'{/for}'+
				'</textarea></p>'+
				'<div class="s4"></div>'
			...
			for(var i=0;i<length;i++){
				...
				$.ajax({
					...
					on: true,
					type:"get",
					url: LOCALHOST_URL+'/web-bin/m/medical/doctor/query_visitOrder_data'+id,
					dataType:"json",
					success: function(data1){
						...
						$("#m"+i).find(".toMedicine").bind("click",function(){
							...
							location.href = LOCALHOST_URL+'/web-bin/m/medical/doctor/to_visit_medicine_page'+data2;
						});	
					},
				});
			}
