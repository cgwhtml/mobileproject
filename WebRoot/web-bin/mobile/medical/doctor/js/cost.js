...
$(function(){
	...
	if(result=="outhospFeeId"){
		$("#top_in").find("div").bind("click",function(){
			location.href = LOCALHOST_URL+'/web-bin/m/medical/doctor/to_visit_case_page'+url;
		});
		...
		$.ajax({
			...
			on: true,
			type:"get",
			url: LOCALHOST_URL+'/web-bin/m/medical/doctor/query_visit_cost_data'+query,
			dataType:"json",
		...
		$("#top_in").find("div").bind("click",function(){
			location.href = LOCALHOST_URL+'/web-bin/m/medical/doctor/to_hosp_case_page'+url;
		});
		...
		$.ajax({
			...
			on: true,
			type:"get",
			url: LOCALHOST_URL+'/web-bin/m/medical/doctor/to_hosp_case_page'+query,
			dataType:"json",
			success: function(data){
				callBack_hosp(data);
			},
...
function callBack_visit(data){
	if(data.data!=null){
		...
		$(".c2").bind("click",function(){
			...
			location.href= LOCALHOST_URL+'/web-bin/m/medical/doctor/to_cost_detail_page'+result;
		});	
	}else if(data.res!=0){
		$(".s1").html(
			'<div class="c2">'+
...
function callBack_hosp(data){
	if(data!=null && data.data!=null){
		...
		$.ajax({
			on: true,
			type:"get",
			url: LOCALHOST_URL+'/web-bin/m/medical/doctor/to_hosp_case_detail_page'+query,
			dataType:"json",
			success: function(datas){
				$(".c2").bind("click",function(){
					...
					location.href= LOCALHOST_URL+'/web-bin/m/medical/doctor/to_cost_detail_page'+json;
				});	
