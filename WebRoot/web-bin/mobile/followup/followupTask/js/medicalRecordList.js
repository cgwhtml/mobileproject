...
$(function(){
    ...
    if(inHosNo!=""){
        ...
        $.ajax({
            on: false,
            type:"get",
            url: LOCALHOST_URL+'/web-bin/m/followup/followupTask/get_medicalRecordList_data?hospCard='+inHosNo+"&hospCode="+hospCode+'&usId='+query.usId+'&accessToken='+query.accessToken+"&cardType=2",
            dataType:"json",
        ...
        $.ajax({
            on: false,
            type:"get",
            url: LOCALHOST_URL+'/web-bin/m/followup/followupTask/get_medicalRecordList_data?hospCard='+visitCardNo+"&hospCode="+hospCode+'&usId='+query.usId+'&accessToken='+query.accessToken+"&cardType=1",
            dataType:"json",
            success: function(data){
                console.log(data);
                num+=1;
                ...
                $.ajax({
                    on: false,
                    type:"get",
                    url: LOCALHOST_URL+'/web-bin/m/followup/followupTask/get_medicalRecordList_data?hospCard='+visitCardNo+"&hospCode="+hospCode+'&usId='+query.usId+'&accessToken='+query.accessToken+"&cardType=2",
                    dataType:"json",
                    success: function(data){
                        console.log(data);
                        num+=1;
...
function dataSort(arr){//时间倒序
    arr.sort(function(a,b){
        return Date.parse(b.beginTime)-Date.parse(a.beginTime);
    });
    console.log(arr);
    return arr;
}
