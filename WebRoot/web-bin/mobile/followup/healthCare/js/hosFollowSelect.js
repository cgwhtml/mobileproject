...
var url=LOCALHOST_URL+'/web-bin/m/followup/healthCare/get_hosFollowDetail_data?usId='+query.usId+'&accessToken='+query.accessToken+'&partnerType='+query.partnerType;
...
$(function(){
    ...
    $("body").on("click",".con_box .list_inner",function(){
        ...
        location.href=LOCALHOST_URL+'/web-bin/m/nosen/followup/form/to_checkForm_page?id='+id+"&app=1&isDoc=1"+"&hugId="+hgId;
    });
    //编辑手机号
        ...
        $.prompt({
            ...
            onOK: function (input) {
                if(!(/^1[34578]\d{9}$/.test(input))){
                    ...
                    if(query.phone==input){
                        isNewPhone=0;
                    }else{
                        $(".footer .phone_num").html(input);
                        phone=input;
                        isNewPhone=1;
                    }
                }
