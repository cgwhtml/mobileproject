...
var Chart = function(){
	...
	return {
		...
		init : function(element){
            ...
            //手机端拖动
            document.getElementById("contentChart").addEventListener("touchstart", c_touchStart, false);
            function c_touchStart(e) {
                e.preventDefault();
                var event = e.touches[0];
                m_pos = Chart.windowToCanvas(canvasObj,event.clientX,event.clientY);
                document.getElementById("contentChart").addEventListener("touchmove", c_touchMove, false);
                document.getElementById("contentChart").addEventListener("touchend", c_touchEnd, false);
            }
            function c_touchMove(e){
            	e.preventDefault();
            	var event = e.touches[0];
            	var pos1 = Chart.windowToCanvas(canvasObj,event.clientX,event.clientY);
                var x = pos1.x - m_pos.x;
                var y = pos1.y - m_pos.y;
                m_pos = pos1;
                offsetX+=Math.floor(x);
                offsetY+=Math.floor(y);
                Chart.draw(element, "drag");
            }
            function c_touchEnd(e){
            	e.preventDefault();
                document.getElementById("contentChart").removeEventListener("touchmove",null,false);
        		document.getElementById("contentChart").removeEventListener("touchend",null,false);
            }
