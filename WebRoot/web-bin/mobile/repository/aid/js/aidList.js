...
$(function(){
	...
	$.ajax({
		on: true,
		type:"post",
		url: LOCALHOST_URL+'/web-bin/m/nosen/repository/aid/query_aidTypeList_data',
		beforeSend:function(){
	...
	$("#partList_result").on("tap",".partItem",function(){
		...
		$.ajax({
			...
			url: LOCALHOST_URL+'/web-bin/m/nosen/repository/aid/query_aidNameList_data?type='+type,
			dataType:"json",
			success: function(data){
	...
	$("#drugList_result").on("tap",".drugItem",function(){
		...
		setTimeout(function(){
			...
			location.href = LOCALHOST_URL+'/web-bin/m/nosen/repository/aid/to_emergencyMsg_page?id='+id;
		}, 200);
	});
	//搜索急救按钮
	$(".input_mes").tap(function(){
		...
		if ($value) {
			$.ajax({
				on: true,
				type:"post",
				url:LOCALHOST_URL+'/web-bin/m/nosen/repository/aid/query_searchAid_data?title='+$value,
				beforeSend:function(){
					//加载中
