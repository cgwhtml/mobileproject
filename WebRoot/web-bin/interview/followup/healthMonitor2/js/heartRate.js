...
var limit1=[60,100];//心率默认正常范围
var limit2=[75,98];//血氧默认正常范围
$(function(){
    var s_data="?healthType=5&docHugId="+query.docHugId;
...
function ajaxCallBack(data){
    if(data.data!=null&&data.data.healthPage){
        ...
        if(h_set) {
            if (h_set.minHR && h_set.maxHR) {
                limit1[0] = h_set.minHR;
                limit1[1] = h_set.maxHR;
            }
            if (h_set.minSaO2 && h_set.maxSaO2) {
                limit2[0] = h_set.minSaO2;
                limit2[1] = h_set.maxSaO2;
            }
        }
		for(var i=0;i<result.length;i++){
			x_data.push(result[i].measuringTime);
            var obj1={};
            obj1.y=parseFloat(result[i].healthValue);
            obj1.p_x=result[i].measuringTime;
            data1.push(obj1);
            var obj2={};
            obj2.y=parseFloat((result[i].instruction?result[i].instruction.split("/")[0]:0));
