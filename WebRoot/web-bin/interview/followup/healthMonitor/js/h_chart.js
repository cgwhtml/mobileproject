...
var Chart = function(){
	...
	return {
		...
		draw : function(element, type){
            ...
            if(offsetX < 0){
            	if(type == 'drag'){
	                ...
	                var data = $("#"+element.substring(5,element.length)).find(".data").val();
		...
		randerValue : function(context, element, ox){
			var data = $("#"+element.substring(5,element.length)).find(".data").val();
			var fetalMoveData = $("#"+element.substring(5,element.length)).find(".fetalMoveData").val();
		...
		randerUterusValue : function(context, element, ox){
			var data = $("#"+element.substring(5,element.length)).find(".uterusData").val();
			if(data.trim() != "" && data != 'null'){
				var dataArray;
				if(data.indexOf("[") != -1 && data.indexOf("]") != -1){
					dataArray = eval('(' + data + ')');
