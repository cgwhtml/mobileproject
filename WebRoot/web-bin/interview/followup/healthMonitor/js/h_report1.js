...
/**********************************************************************************************/
//进度条
var startX, x, aboveX = 0,progress=230; //触摸时的坐标 //滑动的距离  //设一个全局变量记录上一次内部块滑动的位置 //进度条长度-滑块宽度的一半
...
var Report = function(){
	...
	return {
		...
		initMusicPlayer:function(dataObj){
			...
			$("#gestationalWeeks").text(dataObj.data.instruction.split("/")[3]);
			...
			var newTime = new Date(endDate.getTime()+Report.audioDuration*1000); //毫秒转普通时间
		...
		chartCurrentTime : function(element){
			...
			if(absOffsetX > dataArray.length){
			    ...
			    $(".timeNow").text(minute+":"+second);
			    $(".valueNow").text(dataArray[absOffsetX]);
			}
		},
