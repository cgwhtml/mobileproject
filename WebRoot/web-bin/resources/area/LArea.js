...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            ...
            this.fontClass = params.fontClass;
            switch (this.type) {
                case 1:
                case 2:
                    break;
            ...
            function rollGear(target) {
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    if (stopGear) {
                        ...
                        clearInterval(target["int_" + target.id]);
                        $(target).find("."+_self.fontClass).removeClass(_self.fontClass);
                        var index=Math.abs(pos)/2;
                        $(target.childNodes[index]).addClass(_self.fontClass);
                    }
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                ...
                $(gearChild[_self.index].childNodes[gearVal]).addClass(_self.fontClass);
                _self.index++;
                if (_self.index > 2) {
                    _self.index = 0;
                    return;
        ...
        finish: function(e) {
            ...
            var result={
            	...
            	"text":provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''))
            };
