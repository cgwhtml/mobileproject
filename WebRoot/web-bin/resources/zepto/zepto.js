...
//get请求参数转对象
function getRequest(isDecode) {
    var url = window.location.search; //获取url中"?"符后的字串
    isDecode&&(url=decodeURI(url));
...
function IdCardValidate(idCard) {
    if (idCard.length == 15) {
        ...
        return isTrueValidateCodeBy18IdCard(a_idCard);   //进行18位身份证的基本验证和第18位的验证
    } else {
        return false;
    }
}
...
//根据身份证获取相关信息
function personInfoByIdCard(UUserCard,num){
    if(num==1){
        //获取出生日期
...
//截取字符串方法并显示
function substrText(obj,value,length){
    length=parseInt(length);
    if(value!=null&&value.length>length){
        obj.text(value.substring(0,length-1)+"...");
    }else{
        obj.text(value);
    }
}
//判断手机类型是否ios
function queryMobileTypeIsIos(){
